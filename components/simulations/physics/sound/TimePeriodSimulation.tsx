...
export default function TimePeriodSimulation() {
  ...
  useEffect(() => {
    ...
    // Displacement vs time at one position
    ctx.strokeStyle = "rgba(34, 211, 238, 0.9)";
    ctx.lineWidth = 2 * dpr;
    ctx.beginPath();
    const steps = 150;
    for (let i = 0; i <= steps; i++) {
      const timeNorm = i / steps;
      const time = timeNorm * 2;
      const x = marginX + timeNorm * graphW;
      const y = cy - A * Math.sin(omega * (t + time));
    ...
    ctx.fillText("Displacement vs time — one cycle = T", marginX, marginY - 8);
    ctx.fillText(`f = ${formatNum(frequency, 2)} Hz  →  T = 1/f = ${formatNum(period, 2)} s`, marginX, h - 10 * dpr);
  }, [frequency, isAnimating, t, omega]);

  const sidebar = (
    <>
      <h3 className="text-sm font-semibold text-neutral-200 uppercase tracking-wider">Parameters</h3>
      <SliderControl label="Frequency f" value={frequency} min={0.5} max={5} step={0.25} unit="Hz" onChange={setFrequency} color="cyan" />
      <div className="rounded-lg border border-cyan-500/25 bg-cyan-500/10 p-3 text-sm text-neutral-200">
        <div className="text-xs font-medium text-cyan-300 uppercase tracking-wider">T = 1/f</div>
        <div className="font-mono mt-1">T = {formatNum(period, 3)} s</div>
        <p className="text-xs mt-2">Time for one complete oscillation. Higher f → smaller T.</p>
      </div>
    </>
  );

  return (
    <SoundShell title="Time Period" subtitle="f ↔ T inverse relationship shown live." onLaunch={launch} onPause={pause} onReset={reset} paused={isPaused} hasLaunched={hasLaunched} sidebar={sidebar}>
