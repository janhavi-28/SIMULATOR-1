...
export function WorkEnergyShell({
  title,
  subtitle,
  onLaunch,
  onPause,
...
export function SliderControl({
  ...
  return (
    <div className="space-y-1.5">
      ...
      <input type="range" min={min} max={max} step={step} value={value} onChange={(e) => onChange(Number(e.target.value))} className={`w-full h-2.5 rounded-full appearance-none bg-neutral-600 outline-none [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-5 [&::-webkit-slider-thumb]:h-5 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:ring-2 ${thumb}`} title={label} aria-label={label} />
    </div>
  );
}
